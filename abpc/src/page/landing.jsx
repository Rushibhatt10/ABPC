...
const LandingPage = () => {
 ...
 const avatarDataUrl = (name) => {
   ...
   const bg = isDark ? "#111827" : "#E2E8F0";
   const fg = isDark ? "#F8FAFC" : "#0F172A";
   const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256"><rect width="100%" height="100%" fill="${bg}"/><text x="50%" y="54%" text-anchor="middle" dominant-baseline="middle" font-family="Arial" font-size="110" fill="${fg}">${initial}</text></svg>`;
...
useEffect(() => {
 ...
 return (
 <div className={`min-h-dvh w-full overflow-x-hidden safe-bottom-pad transition-colors duration-700 ${isDark ?"bg-black text-white" :"bg-[#faf9f6] text-[#0c0c0c]"}`}>
 {showAnkitPopup && (
 <div
 ...
 >
 <div
 className={`w-full max-w-md rounded-3xl border shadow-2xl p-5 sm:p-6 ${isDark ?"bg-[#0a0a0a] border-white/15 text-white" :"bg-white border-black/10 text-black"}`}
 ...
 {/* ===== NAV ===== */}
 <nav className={`fixed top-0 w-full z-50 flex justify-between items-center px-4 sm:px-6 md:px-8 lg:px-12 pt-[calc(env(safe-area-inset-top)+0.65rem)] pb-3 md:pt-[calc(env(safe-area-inset-top)+0.9rem)] md:pb-4 transition-colors duration-700 ${isDark ?"bg-black/80 backdrop-blur-xl border-b border-white/10" :"bg-[#faf9f6]/80 backdrop-blur-xl border-b border-black/5"}`}>
 {/* Logo */}
 ...
 <div
 className={`fixed inset-0 z-60 flex transition-all duration-500 ${menuOpen ?"opacity-100 pointer-events-auto" :"opacity-0 pointer-events-none"} ${isDark ?"bg-black" :"bg-[#faf9f6]"}`}
 ...
 <div
 className={`hidden md:flex flex-col justify-between w-64 lg:w-80 shrink-0 p-8 md:p-10 border-r transition-transform duration-500 ${menuOpen ?"translate-x-0" :"-translate-x-full"} ${isDark ?"border-white/10 bg-[#0a0a0a]" :"border-black/8 bg-white"}`}
 >
 <div className="mb-4">
 ...
 <button
   type="button"
   onClick={() => {
     navigator.clipboard?.writeText("+91 93744 88004").catch(() => {});
     alert("WhatsApp number copied (offline mode).");
   }}
...
<div className="flex flex-col items-center gap-4 shrink-0 lg:w-1/4">
  ...
  <div
    ...
    group relative ${isDark ? "bg-[#0a0a0a]" : "bg-gray-100"}`}
  >
    <img
      src="/bb.jpeg"
 ...
 <div className={`mt-4 p-6 rounded-2xl border ${isDark ? 'bg-[#0a0a0a] border-white/10' : 'bg-black/5 border-black/10'}`}>
 ...
 {stats.map((s) => (
 <div key={s.label} className={`flex-1 p-4 rounded-2xl border flex flex-col items-center text-center gap-2 transition-all duration-300 ${isDark ?"border-white/10 bg-[#0a0a0a] hover:bg-[#111]" :"border-black/8 bg-white/60 hover:bg-white/90"}`}>
 ...
 <div className="flex flex-col items-center gap-4 shrink-0 lg:w-1/4">
 <div className={`w-48 h-48 md:w-56 md:h-56 xl:w-64 xl:h-64 rounded-full overflow-hidden border border-current/10 shadow-xl group relative ${isDark ?"bg-[#0a0a0a]" :"bg-gray-100"}`}>
 <img src="abp.jpeg" alt="Ankit Bhatt - Co-founder" className="w-full h-full object-cover object-[center_15%] scale-110 transition-transform duration-700 group-hover:scale-125" onError={(e) => { e.currentTarget.src = avatarDataUrl("Ankit Bhatt"); }} />
 </div>
 <div className="text-center">
 ...
 <button
   ...
   className={`flex flex-col items-center gap-2 p-5 rounded-2xl border transition-all duration-300 group ${isDark ?"border-white/10 hover:bg-white/10" :"border-black/10 hover:bg-black/5"}`}
 >
 ...
 {/* ===== GOOGLE MAP ===== */}
 <section id="location" className={`w-full py-16 md:py-24 px-5 md:px-12 ${isDark ?"bg-black" :"bg-[#f1f0ec]"}`}>
 <div className="max-w-7xl mx-auto">
 ...
 {/* ===== FOOTER / SOCIAL ===== */}
 <footer className={`w-full py-12 md:py-16 px-5 md:px-12 ${isDark ?"bg-black" :"bg-[#0c0c0c]"} text-white`}>
 <div className="max-w-7xl mx-auto">
 ...
 {/* Facebook (offline safe) */}
 <button
 type="button"
 onClick={() => {
   navigator.clipboard?.writeText("facebook.com/abpestcontrolsurat").catch(() => {});
   alert("Facebook link copied (offline mode).");
 }}
 ...
 type="button"
 onClick={() => {
   navigator.clipboard?.writeText("instagram.com/abpestcon/").catch(() => {});
   alert("Instagram link copied (offline mode).");
 }}
...
<Link
  to="/rushzzz"
  className="group inline-flex flex-col items-start gap-1 leading-tight transition-all duration-300"
>
  <span className="uppercase tracking-[0.18em] text-sm font-medium text-white group-hover:text-yellow-400">
    Site Developed by Rushi Bhatt
  </span>

  <span className="text-xs tracking-wide text-white/50 group-hover:text-white/80 transition-colors">
    Tap to know more
  </span>

  {/* subtle underline animation */}
  <span className="h-1px w-0 bg-yellow-400 transition-all duration-300 group-hover:w-full"></span>
</Link>
</div>
 </div>
 </div>
